...
#SBATCH -N 8                            # SLURM_JOB_NUM_NODES ==> @course [1-4]
...
for i in {1..10}
 do
  mpirun -n 128 ./jacobi.exe < input
 done

cat slurm* | grep El >> benchmarkmem_0128/8nodeELAPSED.out
cat slurm* | grep MF >> benchmarkmem_0128/8nodeMFLOPS.out
